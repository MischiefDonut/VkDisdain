...
void RenderFrameModels(FModelRenderer* renderer, FLevelLocals* Level, const FSpriteModelFrame *smf, const FState* curState, int curTics, double tic, FTranslationID translation, AActor* actor, HWSprite *spr);
...
void RenderHUDModel(FModelRenderer *renderer, DPSprite *psp, FVector3 translation, FVector3 rotation, FVector3 rotation_pivot, FSpriteModelFrame *smf, double ticFrac)
{
	...
	RenderFrameModels(renderer, playermo->Level, smf, psp->GetState(), psp->GetTics(), ticFrac, trans, psp->Caller, nullptr);
...
bool CalcModelOverrides(int i, const FSpriteModelFrame *smf, DActorModelData* data, const CalcModelFrameInfo &info, ModelDrawInfo &out, bool is_decoupled, HWSprite *spr)
{
	//reset drawinfo
	out.modelid = -1;
	out.animationid = -1;
	...
	else
	{
		...
		out.modelframe = smf->modelframes[i];
		if (info.smfNext) out.modelframenext = info.smfNext->modelframes[i];
		out.skinid = (spr && spr->spr && spr->spr->PT.texture.isValid())? spr->spr->PT.texture : smf->skinIDs[i];
	}
...
void RenderFrameModels(FModelRenderer *renderer, FLevelLocals *Level, const FSpriteModelFrame *smf, const FState *curState, int curTics, double ticFrac, FTranslationID translation, AActor* actor, HWSprite *spr)
{
	double tic = Level->totaltime;
	if ((ConsoleState == c_up || ConsoleState == c_rising) && (menuactive == MENU_Off || menuactive == MENU_OnNoPause) && (!actor || !actor->isFrozen()))
	{
	...
	{
		if (CalcModelOverrides(i, smf, modelData, frameinfo, drawinfo, is_decoupled, spr))
		{
